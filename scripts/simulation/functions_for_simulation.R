...
sample_size_d <- 
  ...
  function(data, power = .8, shrinkage = .25) {
    ...
    bb <- sampleSizeReplicationSuccess(zo = data$zo, power = power, level = 0.05, type = "golden", 
...
get_summary_study_rep_pSceptical <- 
  ...
  function(study_data, alpha_s = 0.05) {
    ...
    pScep <- pSceptical(zo = study_summary$zo, 
                        zr = study_summary$zr, 
                        c = study_summary$rep_ss/study_summary$orig_ss,
                        alternative = "one.sided", 
                        type = "golden")
